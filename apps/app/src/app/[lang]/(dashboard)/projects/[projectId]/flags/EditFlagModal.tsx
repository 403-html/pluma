...
import { updateFlag, type FeatureFlag } from '@/lib/api/flags';
...
export function EditFlagModal({
  ...
  function handleKeyBlur() {
    ...
    if (trimmedKey !== flag.key && existingKeys.includes(trimmedKey)) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const data: { key?: string; name?: string; description?: string | null } = {};
    if (key !== flag.key) {
      data.key = key;
    }
    if (name !== flag.name) {
    ...
    const result = await updateFlag(flag.id, data);
    if (result.ok) {
      onSuccess();
    } else {
